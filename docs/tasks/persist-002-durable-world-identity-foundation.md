...
- **Open obligations:**
  ...
  - **PERSIST002-NET-02 — MERGED / CLOSED** (subsequent repository-stage closure). Public persistence rollout remains **BLOCKED**. [ROLLOUT-01](persist-002-rollout-01-staging-readiness.md) now prepares the persistent deployment contract, immutable DB operations and evidence procedures. Live final-topology/two-source acceptance and a separate PA deployment GO remain required. Earlier dated NET-02 OPEN wording below is historical; repository merge is not deployment permission.
