...
export async function verifyDumpIntegrity(dumpPath: string, manifestPath: string): Promise<BackupManifest> {
  ...
  if (actualSha256 !== manifest.dumpSha256) {
    throw new PersistenceToolError(
      `Dump integrity check failed: manifest recorded ${manifest.dumpSha256}, actual dump SHA-256 is ${actualSha256}.`
    );
  }
...
export async function restoreAndVerify(options: RestoreVerifyOptions): Promise<RestoreVerificationResult> {
  ...
  try {
    ...
    }>('SELECT world_id, world_slug, state_revision, domain_version FROM worlds WHERE world_slug = $1', [
...
async function main(): Promise<number> {
  ...
  try {
    const result = await restoreAndVerify({ dumpPath, manifestPath, targetMigratorUrl, grantsSqlPath });
